...
function preprocess(src,context,type) {
  ...
  if (delim[type].ifdef) {
    rv = rv.replace(getRegex(type,'ifdef'),function(match,test,include){
      console.log(testForMultiples(test, context));
      test = (test || '').trim();
      return testForMultiples(test, context) !== 'undefined' ? include : '';
    });
  }
...
function testForMultiples(test, context) {
  var returnStr = '', 
  multiples = test.split(' '),
  reg = new RegExp('[A-Za-z]');

  if(multiples.length === 1) {
    return typeof context[multiples[0]];
  }
  
  multiples.forEach(function(testStr) {
    if(reg.test(testStr)) {
      returnStr += context[testStr]+ ' '
    } else {
      returnStr += testStr + " "
    }
  });

  if(new Function('return ' + returnStr)()) {
    return "true";
  } else {
    return 'undefined';
  }
